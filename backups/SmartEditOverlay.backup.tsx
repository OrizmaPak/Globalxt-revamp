...
const SmartEditOverlay = () => {
  ...
  // Listen for custom element click events
  useEffect(() => {
    if (!isSmartEditMode) return;
    
    const handleCustomElementClick = (e: CustomEvent) => {
      const { element, content, type, tagName, component } = e.detail;
      ...
      // Directly open edit modal instead of choice modal
      setShowEditModal(true);
    };
    
    document.addEventListener('smartEditElementClicked', handleCustomElementClick as EventListener);
    
    return () => {
      document.removeEventListener('smartEditElementClicked', handleCustomElementClick as EventListener);
    };
  }, [isSmartEditMode]);
  ...
  const handleModalSave = (newContent: string) => {
    if (selectedElement) {
      // Find the matching editable element by content and element reference
      const matchingElement = editableElements.find(el => 
        el.element === selectedElement.htmlElement && 
        el.content === selectedElement.content
      );
      
      if (matchingElement) {
        // Use the existing updateElement function
        updateElement(matchingElement.id, newContent);
        
        console.log('Content updated via existing system:', {
        ...
        // Fallback: Update DOM directly if no matching element found
        selectedElement.htmlElement.textContent = newContent;
        console.log('Content updated directly (no match found):', {
          element: selectedElement.tagName,
          oldContent: selectedElement.content,
          newContent: newContent,
          component: selectedElement.component
  ...
  return (
    <>
      ...
      {/* Debug info is now console-only - no UI blocking */}
